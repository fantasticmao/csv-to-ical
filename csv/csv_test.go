...
func validate(t *testing.T, events []Event) {
	...
	assert.Equal(t, config.Solar, events[0].CalendarType)
	...
	assert.Equal(t, config.Solar, events[1].CalendarType)
	...
	assert.Equal(t, config.Lunar, events[2].CalendarType)
	...
	assert.Equal(t, config.Lunar, events[3].CalendarType)
	...
	assert.Equal(t, config.BirthdayLunar, events[7].CalendarType)
}
